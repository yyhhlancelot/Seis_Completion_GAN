...
def create_blocky_image(x, local_random, min_resize_ratio):
    ratio = local_random.rand() * (0.95 - min_resize_ratio) + min_resize_ratio
    # print('ratio : ', ratio)
    tmp = sp.misc.imresize(x, ratio, interp='nearest', mode = None)
    # tmp = sp.misc.imresize(x, ratio)
    y = sp.misc.imresize(tmp, [x.shape[0], x.shape[1]], interp='nearest').astype(float) / 255.0

    return y

def actually_add_noise(x, local_random, 
                   std=0.6,
                  uniform_max=3.464,
                  continuous_noise=0,
    ...
    # if continuous_noise > 0:
        # rand_std = local_random.rand() * (std - 0.05) + 0.05
        # noise = local_random.randn(x_shape[0], x_shape[1]) * rand_std
    # else:
        # noise = local_random.randn(x_shape[0], x_shape[1]) * std
...
def add_noise(x, local_random,
    ...
    r = local_random.rand() # 0-1之间的浮点数
    if r < prob_block:
        y = create_blocky_image(x, local_random, min_resize_ratio=0.2)

    result = actually_add_noise(y, local_random)
